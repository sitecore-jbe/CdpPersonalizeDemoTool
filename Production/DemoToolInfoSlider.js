...
(function () {
    ...
    */
    ...
    function ComposeDemoToolProperty(propertyDefinitionPath, parentHtmlElement) {
        ...
        if ((createIfEmptyValue || displayValue) && display) {
            ...
            if (propertyDefinition.Link) {
                ...
                propertyValue = AppendElementAsChild(property, 'a', { id: property.id + "Link", classList: "propertyvalue", href: link, target: DEMOTOOL_SITECORECDP_TARGET_NAME, innerText: JSON.stringify(displayValue) });
                ...
                propertyValue = AppendElementAsChild(property, 'label', { id: property.id + "Value", classList: "propertyvalue", innerHTML: JSON.stringify(displayValue) });
                if (propertyDefinition.Suffix) {
                    AppendSuffix(propertyValue, propertyDefinition.Suffix, dataPath);
                }
            }
    ...
    function InitializeInfoSlider() {
        ...
        var InfoSliderBodyHeaderCdpLogoImage = AppendElementAsChild(InfoSliderBodyHeaderRightTop, "img", { id: InfoSliderBodyHeaderRightTop.id + "Image", src: CDPLOGO_URL, classList: "cdpLogo" });
        var InfoSliderBodyHeaderAndLabel = AppendElementAsChild(InfoSliderBodyHeaderRightTop, "label", { id: InfoSliderBodyHeaderRightTop.id + "Label", innerText: " & ", classList: "and"  });
        var InfoSliderBodyHeaderPersonalizeLogoImage = AppendElementAsChild(InfoSliderBodyHeaderRightTop, "img", { id: InfoSliderBodyHeaderRightTop.id + "Image", src: PERSONALIZELOGO_URL, classList: "personalizeLogo" });
    ...
    function KeyPress(e) {
        ...
        // CTRL + B send identity event
        if (evtobj.keyCode == 66 && evtobj.ctrlKey) {
            GetIPGeolocation().then(function (response) {

                console.debug(CONSOLE_LOG_PREFIX + "CTRL + B pressed = Send more complete identity event");
                var identityEvent = {
                    "browser_id": Boxever.getID(),
                    "channel": SITECORECDP_CHANNEL,
                    ...
                    "pos": SITECORECDP_POINT_OF_SALE,
                    "email": document.querySelector("input[type='email'][data-sc-field-name='Email']").value,
                    "firstname": document.querySelector("input[type='text'][data-sc-field-name='First Name Text']").value || SITECORECDP_DEFAULT_FIRSTNAME_VALUE,
                    "lastname": document.querySelector("input[type='text'][data-sc-field-name='Last Name Text']").value || SITECORECDP_DEFAULT_LASTNAME_VALUE,
                    "dateOfBirth": SITECORECDP_DEFAULT_DATEOFBIRTH_VALUE,
                    "phoneNumbers": [document.querySelector("input[type='tel'][data-sc-field-name='Telephone']").value],
                    "country": response.country_code2,
                    "city": response.city,
                    "postcode": response.zipcode,
                    "identifiers": [{
                        "provider": SITECORECDP_IDENTITY_PROVIDER,
                        "id": document.querySelector("input[type='email'][data-sc-field-name='Email']").value
                    }]
                };
                ...
                var segmentationData = {
                    "key": "Segmentation",
                    "format": "HTML",
                    "cruiseExperienceText": document.querySelector("select[data-sc-field-name='Identify List']").options[document.querySelector("select[data-sc-field-name='Identify List']").selectedIndex].text,
                    "cruiseExperienceValue": document.querySelector("select[data-sc-field-name='Identify List']").value,
                    "destinationPreferenceText": document.querySelector("select[data-sc-field-name='Destination List']").options[document.querySelector("select[data-sc-field-name='Destination List']").selectedIndex].text,
                    "destinationPreferenceValue": document.querySelector("select[data-sc-field-name='Destination List']").value
                };
                SendGuestDataExtensions("ext", segmentationData);
                //    });
                //});
            });
        }
        ...
        // CTRL + M send identity event
        if (evtobj.keyCode == 77 && evtobj.ctrlKey) {
            GetIPGeolocation().then(function (response) {
                console.debug(CONSOLE_LOG_PREFIX + "CTRL + M pressed = Send identity event");
                var identityEvent = {
                    "browser_id": Boxever.getID(),
                    "channel": SITECORECDP_CHANNEL,
                    "type": SITECORECDP_IDENTITY_TYPE,
                    ...
                    "page": window.location.pathname,
                    "pos": SITECORECDP_POINT_OF_SALE,
                    "email": document.getElementById("your-email").value || SITECORECDP_DEFAULT_EMAIL_VALUE,
                    "country": response.country_code2,
                    "city": response.city,
                    "postcode": response.zipcode,
                    "identifiers": [{
                        "provider": SITECORECDP_IDENTITY_PROVIDER,
                        "id": document.getElementById("your-email").value || SITECORECDP_DEFAULT_EMAIL_VALUE
                    }]
                };
                SendTrackingDataToSitecoreCdp(identityEvent);
            });
        ...
        if (evtobj.keyCode == 49 && evtobj.ctrlKey) {
            console.debug(CONSOLE_LOG_PREFIX + "Starting: Trigger Email Next Best Offers Event");

            var customEvent1 = {
                "browser_id": Boxever.getID(),
                "channel": SITECORECDP_CHANNEL,
                "type": "EMAIL_NEXT_BEST_OFFERS",
                "pos": SITECORECDP_POINT_OF_SALE
            };
            SendTrackingDataToSitecoreCdp(customEvent1);

            console.debug(CONSOLE_LOG_PREFIX + "Ended: Trigger email Next Best Offers Event");
        }

        // CTRL + 9 - custom event 3 - control-3 starts USE CASE 3
        if (evtobj.keyCode == 57 && evtobj.ctrlKey) {
